...
describe('xAI response parsing', () => {
  ...
  test('rejects a bad confidence value the schema should have prevented', () => {
    const result = xaiProvider.parseResponse(
      completion({
        ...okMessage,
        content: JSON.stringify({ ...interpretation, confidence: 'certain' })
      })
    )
...
describe('xAI request', () => {
  const args = {
    context: { jobDescription: 'staff engineer', notes: '' },
    turns: [],
